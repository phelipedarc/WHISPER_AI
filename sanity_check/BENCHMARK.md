...
Sampler budgets (fast benchmark tier; see `sanity_check/benchmark_kilonova_modes.py`):
...
for m in flux magnitude; do for s in abc mcmc snpe; do
    python sanity_check/benchmark_kilonova_modes.py fit $m $s &
done; done; wait
python sanity_check/benchmark_kilonova_modes.py plot
```

Each `fit` run writes its own `sanity_check/figures/kilonova_bench_<mode>_<sampler>.{json,npz}` (so the six are
...
*misspecified* `mck19` (see `sanity_check/sanity_mck19_modes.py`), where flux- and magnitude-space pull to
...
sampler at once (`sanity_check/corner_kilonova_benchmark.py`, via the built-in `wp.plot_corner`):
...
(`sanity_check/figures/at2017gfo_corner_flux.png`, `python sanity_check/corner_kilonova_benchmark.py flux`) tells the
same story.

## SNPE on GPU vs CPU

`fit_SNPE(device='cuda'|'auto'|...)` trains the neural density estimator on a GPU. The GPU accelerates
**training**, not the (CPU) simulator, so `sanity_check/benchmark_snpe_device.py` isolates it on the cheap
...
For an **end-to-end recovery benchmark on synthetic data with known ground truth** — MCMC,
noise-matched ABC/ABC-SMC and GPU neural SBI (NPE with **MDN and NSF** density estimators plus a
5-round SNPE benchmark of **no-embedding vs MLP vs TCN** embeddings, all conditioning on the stacked
`(value, err, time, band)` tuple with GPU-vectorized simulation, one GPU per method in parallel)
fitting mocks `M(t, θ)+noise`, timed, with recovery z-scores, posterior-predictive checks and
Simulation-Based Calibration — see
[`figures/REPORT.md`](figures/REPORT.md) (generated by
`sanity_check/sanity_check.py`). Headlines: every method recovers the Bazin (2009) supernova truth within
1σ; **NPE-NSF joins exact MCMC as formally SBC-calibrated**; amortized NPE infers a **new** object in
~10–100 ms after one training (vs a full refit for every likelihood method); and the TCN embedding
gives the best sequential-SNPE recovery while the MLP is the fastest.
