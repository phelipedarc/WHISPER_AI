...
NPE-GPU, SNPE-GPU), and check each **recovers the truth** with **calibrated uncertainties** — while
timing everything. Two mocks: a 4-parameter damped sinusoid (detailed showcase) and a 2/4/6-parameter
Gaussian-pulse family (dimensionality sweep). Statistics: per-parameter z-score + credible-interval
coverage, posterior-predictive checks (reduced χ² + Bayesian p-value), and Simulation-Based Calibration
(rank uniformity). Outputs land in ``docs/figures/sanity_check/``.

    # one config at a time (parallel-friendly), then render:
    python scripts/sanity_check.py fit damped_sine mcmc
    python scripts/sanity_check.py sbc damped_sine mcmc
...
DATA_SEED = 1234                   # seed for the showcase (damped_sine) noise realisation
SWEEP_SEED = 6                     # representative draw for the gp2/gp4/gp6 sweep (worst |z_fisher|~0.9)
...
# mock name -> {predict, params, prior, truth, tmax, npts}
MOCKS = {
    "damped_sine": {
        ...
        "tmax": 16.0, "npts": 100, "seed": DATA_SEED, "log": [],
    },
}
for _k in (1, 2, 3):
...
# sampler key -> (label, fit function, kwargs). NPE = 1 round; SNPE = 10 rounds (both GPU).
SAMPLERS = {
...
SBC_L = {"mcmc": 100, "abc": 100, "abc_smc": 60, "npe": 200}   # per-method realisations (SNPE: see docs)
...
def fit(mock, sampler):
    ...
    json.dump({
        ...
        "info": {k: res.info.get(k) for k in ("converged", "mean_acceptance_fraction",
                                              "mean_autocorr_time", "device") if k in res.info},
...
def sbc(mock, sampler):
    ...
    if sampler == "npe":
        ...
        import torch
        ref = _make_lc(mock, spec["truth"], DATA_SEED)
