...
For an **end-to-end recovery benchmark on synthetic data with known ground truth** — all five samplers
(MCMC, ABC, ABC-SMC, NPE-GPU, SNPE-GPU) fitting mocks `M(t, θ)+noise`, timed, with recovery z-scores,
...
`scripts/sanity_check.py`). It confirms every sampler recovers the injected parameters within ~2σ and
uses SBC to flag which posteriors are calibrated.
